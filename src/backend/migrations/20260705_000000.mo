...
module {
  /// Previous actor shape (from the frozen 20260629_000000 init migration).
  type OldActor = {};

  /// User role — must be inlined; no project imports in migrations.
  type UserRole = { #admin; #user; #guest };

  /// Access control state — mirrors `AccessControl.AccessControlState`
  /// but inlined here. Migration files may only import from `mo:core/...`.
  type AccessControlState = {
    var adminAssigned : Bool;
    userRoles : Map.Map<Principal, UserRole>;
  };

  /// Article status — must be inlined; no project imports in migrations.
  type ArticleStatus = { #draft; #published };

  /// Article record — mirrors `types/articles.mo` but inlined here with
  /// `coverImage : ?Blob` (ExternalBlob is `Blob`). Migration files may
  /// only import from `mo:core/...`.
  type Article = {
    slug : Text;
    title : Text;
    excerpt : Text;
  ...
  public func migration(old : OldActor) : NewActor {
    ...
    { accessControlState = { var adminAssigned = false; userRoles = Map.empty() }; articles };
  };
};
